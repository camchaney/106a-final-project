...
def rotation_3d(omega, theta):
    ...
    w_hat = skew_3d(omega)
    first_term = w_hat / w_norm * np.sin(w_norm * theta)
    second_term = (w_hat @ w_hat) / (w_norm**2) * (1 - np.cos(w_norm * theta))
    out = np.eye(3) + first_term + second_term
    return out
    # YOUR CODE HERE
...
def homog_3d(xi, theta):
    ...
    if np.any(omega):
        top_left = rotation_3d(omega, theta)
        top_right = (1 / (np.linalg.norm(omega)**2)) *\
                    (
                        (np.eye(3) - top_left) @\
                        (skew_3d(omega) @ v) +\
                        (omega @ omega.T @ v * theta)
                    )
    else:
        top_left = np.eye(3)
        top_right = v * theta

    bottom_left = np.zeros((1, 3))
    bottom_right = np.ones(1).reshape((1, 1))
    out = np.vstack((
        np.hstack((
            top_left,
            top_right,
